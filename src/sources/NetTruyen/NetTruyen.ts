...
export class NetTruyen extends ZenSource {
  id = 'nettruyen';
  name = 'NátTruyen';
  icon = 'Nettruyen.png';
  description = 'Mọi người chê rác nhưng nó load ảnh nhanh.';
  ...
  getSection = (sectionId: string, page: number) => {
    const includedTags: Tag[] = [{ id: '35', label: 'romance' }];
    const excludedTags: Tag[] = '1,2,15,16,24,25,40,42,45,46,58,59'
      .split(',')
      .map((id) => ({ label: '', id }));
    switch (sectionId) {
      case 'top':
        return this.getViewMoreItems('hot', { page });
      case 'new_romcom':
      ...
      default:
        return this.getViewMoreItems(sectionId, { page });
    }
  };
  ...
  async getChapterDetails(mangaId: string, chapterId: string): Promise<ChapterDetails> {
    const request = createRequestObject({
      url: `${DOMAIN}/truyen-tranh/${mangaId.split('-').slice(0, -1).join('-')}/abc/${chapterId}`,
      method: 'GET',
    });
    const data = await this.requestManager.schedule(request, 1);
    const $ = this.cheerio.load(data.data);
    ...
    const request = createRequestObject({
      url: query.title
        ...
        : url +
          '/tim-truyen-nang-cao' +
          encodeURI(
            `?keyword=${query.title ?? ''}&genres=${search.genres}&notgenres=${
              search.notgenres
            }&gender=${search.gender}&status=${search.status}&minchapter=${
              search.minchapter
            }&sort=${search.sort}&page=${page}`
          ),
      method: 'GET',
    });
    const data = await this.requestManager.schedule(request, 1);
